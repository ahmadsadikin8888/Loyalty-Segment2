...
class YbsService extends CI_Controller
{
	...
	public function get_dok($data)
	{
		...
		// if($xx[0]==$this->_token && $this->_user_id==1){
		$name	= "./temp_upload/dokument/" . $data;
		// Quick check to verify that the file exists
		if (!file_exists($name)) die("File not found");
		$this->load->helper('download');
		$f = file_get_contents($name);
		force_download($this->_token . $data, $f);
		// }
	...
	public function get_imagena($data)
	{
		//$this->_token.$this->_separator_a.user_picture
		$this->load->model('Custom_model/Sms_tracking_model', 'sms_tracking');
		$explode = explode('.', $data);
		$this->sms_tracking->edit(array("id" => $explode[0]), array("status" => 1));
